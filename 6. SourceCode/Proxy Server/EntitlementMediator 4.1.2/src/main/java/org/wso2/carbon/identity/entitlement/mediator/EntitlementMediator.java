...
public class EntitlementMediator extends AbstractMediator implements ManagedLifecycle 
{
    ...
    public boolean mediate(MessageContext synCtx) {
        ...
        try {
            userName = callback.getCloud4allToken(synCtx);
        	...
        	serviceName = "PreferencesServer";
        	action = "read";

            if (userName == null) {
                throw new SynapseException(
                        "MODIFICAT!! user name not provided for the Entitlement mediator - can't proceed");
            }
           ...
           //if (env == null) {
            ...
            //}
                ...
                env[0] = callback.getCloud4allIDApp(synCtx);
                
                env[1] = callback.getCloud4allIP(synCtx);
			...
			// marcel!!
                ...
                System.out.println("Env[0] is IDAPP : " + env[0]);
                System.out.println("Env[1] is IP: " + env[1]);
            ...
            tempArr[2] = new Attribute("urn:oasis:names:tc:xacml:1.0:attribute-category:resource", "urn:oasis:names:tc:xacml:1.0:resource:resource-id", ProxyConstants.DEFAULT_DATA_TYPE, resourceName);
            for(int i=0;i<envLength;i++){
            	//i=0 es IDApp
            	if (i==0)
            	{
            		System.out.println("Posem el Env[0] : " + env[0]);
                    tempArr[3+i]= new Attribute("urn:oasis:names:tc:xacml:1.0:attribute-category:environment", "urn:oasis:names:tc:xacml:1.0:environment:IDApp", ProxyConstants.DEFAULT_DATA_TYPE, env[0]);
                }
            	if (i==1)
            	{
            		System.out.println("Posem el Env[1] : " + env[1]);
                    tempArr[3+i]= new Attribute("urn:oasis:names:tc:xacml:1.0:attribute-category:environment", "urn:oasis:names:tc:xacml:1.0:environment:IP", ProxyConstants.DEFAULT_DATA_TYPE, env[1]);
            	}
            }     
			...
			if (decisionString != null) {
				...
				if (decisionElement.getNamespace() != null) {
					nameSpace = decisionElement.getNamespace().getNamespaceURI();
				}
				if (nameSpace != null && nameSpace.trim().length() > 0) {
					if (decisionElement.getFirstChildWithName(new QName(nameSpace, "Result")) != null) {
					...
					if (decisionElement.getFirstChildWithName(new QName("Result")) != null) {
  ...
  /**
	...
	public String getRemoteServicePassword() {
		if (!remoteServicePassword.startsWith("enc:")) {
			try {
				return "enc:" + CryptoUtil.getDefaultCryptoUtil().encryptAndBase64Encode(remoteServicePassword.getBytes());
